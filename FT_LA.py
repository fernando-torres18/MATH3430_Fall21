...
def scalar_vec_multi(vec_a: list[float], scalar_a: float) -> list:
    ...
    for index in range(len(vec_a)):
        result.append(vec_a[index] * scalar_a)
    return result
...
def mat_vec_multi(matrix: list[float], vector:list[float]) -> list[list[float]]:
    ...
    """
    matrix_result: list[list[float]] = [0 for elements in matrix]
    for index in range(len(vector)):
        matrix_result[index] = scalar_vec_multi(matrix[index], vector[index])
        result = add_vectors(matrix_result[0], matrix_result[1])
    for index in range(2, len(matrix_result)):
        result = add_vectors(result, matrix_result[index])
    return result
