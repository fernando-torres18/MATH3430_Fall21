...
def stable_gram_schmidt(matrix_a: list[list]) -> list[list]:
    ...
    for outer_index in range(len(matrix_a)):
        R[outer_index][outer_index] = FT_LA.p_norm((V[outer_index]))
        Q.append(FT_LA.scalar_vec_multi((V[outer_index]), (1 / R[outer_index][outer_index])))
        for inner_index in range(outer_index, len(matrix_a)):
            R[inner_index][outer_index] = FT_LA.inner_product(Q[outer_index], V[inner_index])
            s = FT_LA.scalar_vec_multi(Q[outer_index], -R[inner_index][outer_index])
            V[inner_index] = FT_LA.add_vectors(V[inner_index], s)
return [Q, R]
